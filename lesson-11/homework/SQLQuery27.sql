Question: Retrieve all employee names along with their corresponding department names.
 ...
 from Employee e
 inner join Employee m on e.EmployeeID = m.ManagerID
 ...
 from Colors c 
 inner join Sizes s on c.ColorID = s.SizeID
 ---
 ...
 select m.Title, m.ReleaseYear, a.Name
 from Movies m
 ...
 select p.ProductName, sum(s.Quantity * p.Price) as TotalQuantity
 from Products p 
